...
namespace MyBlog.Areas.Identity.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public RoleController(ILogger<RoleController> logger, RoleManager<IdentityRole> roleManager, MyBlogDbContext context, UserManager<AppUser> userManager)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
           ...
           var rolesFromDb = await _roleManager.Roles.OrderBy(r => r.Name).ToListAsync();

           var listRolesWithClaim = new List<RoleWithClaimsModel>();
           ...
           foreach (var role in rolesFromDb)
           {
               ...
               var roleModel = new RoleWithClaimsModel()
               {
                   Name = role.Name,
                   Id = role.Id,
                   Claims = claimsString.ToArray()
               };
               listRolesWithClaim.Add(roleModel);
           }

            return View(listRolesWithClaim);
        } 
        ...
        public async Task<IActionResult> CreateAsync(CreateRoleModel model)
        {
            ...
            if (result.Succeeded)
            {
                StatusMessage = $"Bạn vừa tạo role mới role: {model.Name}";
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditConfirmAsync(string roleId, [Bind()]EditRoleModel model)
        {
            if (roleId == null) return NotFound("Không tìm thấy role");
            var role = await _roleManager.FindByIdAsync(roleId);
            if (role == null) return NotFound("Không tìm thấy role");
            ...
            if (result.Succeeded)
            {
                StatusMessage = $"Bạn vừa cập nhật role: {model.Name}";
                return RedirectToAction(nameof(Index));
            }
            else
            {
        ...
        public async Task<IActionResult> EditRoleClaim(int claimid, [Bind("ClaimType", "ClaimValue")]EditClaimModel Input)
        {
            ...
            StatusMessage = $"Vừa cập nhật claim: {Input.ClaimType}";
